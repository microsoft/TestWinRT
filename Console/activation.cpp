...
int32_t __stdcall WINRT_RoGetActivationFactory(void* classId, guid const& iid, void** factory) noexcept
{
    *factory = nullptr;
    ...
    if (starts_with(name, L"TestComponent."))
    {
        library = LoadLibraryW(L"TestComponent.dll");
    }
    else
    {
    ...
    com_ptr<Windows::Foundation::IActivationFactory> activation_factory;
    ...
    if (iid != guid_of<Windows::Foundation::IActivationFactory>())
    {
